...
const useLeaveTypeData = () => {
  ...
  const updateLeaveTypeStatus = async (id, newStatus) => {
    try {
      const response = await axios.put(`http://localhost:9000/api/leave-update-status/${id}`, { leaveStatus: newStatus }, {
        headers: {
      ...
      if (response.data.success) {
        ...
        setTimeout(() => {
          ...
          const updatedData = leaveTypeData.map((item) =>
            item.id === id ? { ...item, leaveStatus: newStatus } : item
          );
  ...
  const addLeaveType = async (newLeaveType) => {
    try {
      const response = await axios.post("http://localhost:9000/api/add-leaveType", {
        ...newLeaveType, companyId: authToken.companyId
      }, {
        headers: {
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        setLeaveTypeData((prevLeave) => {
          return prevLeave.map((leave) =>
            leave.id === updateLeaveType.id ? updateLeaveType : leave
          );
        });
