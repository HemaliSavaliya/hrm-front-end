        ...
        const fetchUserList = async () => {
            try {
                ...
                if (authToken?.role === "Admin") {
                    // If the logged-in-use is and admin, filter for HR and employee roles
                    filteredUsers = response.data.filter(roleUser => roleUser.role !== 'Admin' && (roleUser.role === 'HR' || roleUser.role === 'Employee'));
                } else {
                    // If the logged-in user is not an admin, filter for employee role
                    filteredUsers = response.data.filter(roleUser => roleUser.role !== 'Admin' && roleUser.role === 'Employee');
                }
