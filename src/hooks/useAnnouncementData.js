...
const useAnnouncementData = () => {
    ...
    const addAnnouncement = async (newAnno) => {
        try {
            const response = await axios.post("http://localhost:9000/api/add-announcement", {
                ...newAnno, companyId: authToken.companyId
            }, {
                headers: {
        ...
        try {
            ...
            setTimeout(async () => {
                ...
                setAnnouncementData((prevAnno) => {
                    return prevAnno.map((anno) =>
                        anno.id === updateAnno.id ? updateAnno : anno
                    );
                });
