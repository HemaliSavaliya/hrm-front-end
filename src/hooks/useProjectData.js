...
const useProjectData = () => {
    ...
    const updateProjectStatus = async (id, newStatus) => {
        try {
            const response = await axios.put(`http://localhost:9000/api/update-status/${id}`, { status: newStatus }, {
                headers: {
    ...
    const addProjects = async (newProject) => {
        try {
            const response = await axios.post("http://localhost:9000/api/add-projects", {
                ...newProject, companyId: authToken.companyId
            }, {
                headers: {
