  ...
  const updateDepartmentStatus = async (id, newStatus) => {
    try {
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_URL}/department-update-status/${id}`,
        { status: newStatus },
        {
          headers: {
      ...
      if (response.data.success) {
        ...
        setTimeout(() => {
          ...
          const updatedData = departmentData.map(item => (item.id === id ? { ...item, status: newStatus } : item))
  ...
  const addDepartments = async newDepartment => {
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_URL}/add-department`,
        {
          ...newDepartment,
      ...
      if (response.status === 200) {
        ...
        setTimeout(async () => {
          ...
          setDepartmentData(prevDep => {
            return prevDep.map(depart => (depart.id === department.id ? department : depart))
          })
