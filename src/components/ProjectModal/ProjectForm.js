import { Button, DialogContentText, Grid, Divider, MenuItem, TextField, InputLabel, Typography, CardContent, CardActions, FormControl, Select, Autocomplete, Chip } from '@mui/material';
...
const ProjectForm = ({ handleClose, editProjectId, setOpen, projectData, addProjects, editProjects }) => {
  const { formData, handleInputChange, handleImageChange, errors, validateForm, setFormData, initialFormValue, handleTeamMembersChange } = ProjectFormLogic(projectData, editProjectId);
            ...
            <Grid container spacing={5}>
              {!isInEditMode && (
                <>
                  <Grid item xs={12} sm={12}>
                    ...
                    />
                    {errors.projectName && <Typography sx={{ color: "#FF4433", fontSize: "13px", fontWeight: "lighter", pt: 1 }}>{errors.projectName}</Typography>}
                  </Grid>
                  <Grid item xs={12} sm={6}>
                    <TextField
                      ...
                      onChange={handleInputChange}
                    />
                    {errors.clientName && <Typography sx={{ color: "#FF4433", fontSize: "13px", fontWeight: "lighter", pt: 1 }}>{errors.clientName}</Typography>}
                  </Grid>
                </>
              )}
              <Grid item xs={12} sm={6}>
              ...
              {!isInEditMode && (
                <Grid item xs={12} sm={6}>
                  ...
                  />
                  {errors.startDate && <Typography sx={{ color: "#FF4433", fontSize: "13px", fontWeight: "lighter", pt: 1 }}>{errors.startDate}</Typography>}
                </Grid>
              )}
              <Grid item xs={12} sm={6}>
                <TextField
              ...
              <Grid item xs={12} sm={6}>
                ...
                />
                {errors.teamMembers && <Typography sx={{ color: "#FF4433", fontSize: "13px", fontWeight: "lighter", pt: 1 }}>{errors.teamMembers}</Typography>}
              </Grid>
              <Grid item xs={12} sm={12}>
                <div
                  id="document"
